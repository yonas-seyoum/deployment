...
type Props = {
  onUploaded: (data: Partial<ResumeData>) => void;
};

export default function UploadResume({ onUploaded }: Props) {
  const inputRef = useRef<HTMLInputElement>(null);

  const [uploaded, setUploaded] = useState<ResumeFile | null>(null);
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      const response = await resumeApi.uploadResume(file, (percent) =>
        setProgress(percent)
      );
      onUploaded(response);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {uploaded && (
        <div className="border rounded-lg p-4 bg-card space-y-3 h-full">
